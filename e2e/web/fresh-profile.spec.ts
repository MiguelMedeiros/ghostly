...
for (const mobile of [false, true]) {
  test(`a fresh profile opens without the error screen, and the sidebar works${mobile ? " (phone)" : ""}`, { tag: ["@feature:app.home", "@feature:app.navigation"] }, async ({ browser, relay, baseURL }) => {
    ...
    const places = mobile
      ? ["Wallet", "Services", "Settings"].map((name) => page.getByTestId("mobile-tabs").getByRole("button", { name }))
      : ["account-profile", "account-services", "account-settings", "wallet-chip"].map((id) => page.getByTestId(id));
