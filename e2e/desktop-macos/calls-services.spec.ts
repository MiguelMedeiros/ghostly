  ...
  try {
    ...
    await test.step("DHT only: the call buttons are off and say why, and come back live", async () => {
      const dhtOnly = async (p: DesktopPerson, on: boolean) => {
        ...
        // DHT only is one of the connection choices; off goes back to Automatic.
        const choice = '[data-testid="connection-option-dht"]', back = '[data-testid="connection-option-auto"]';
