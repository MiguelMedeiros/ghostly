...
/** Shares one of mine with this chat's contact and waits until the contact's app verified it. */
export async function shareIdentity(peer: Peer, which?: string | RegExp, { verified = true } = {}) {
  await openIdentities(peer);
  const mine = await turnMine(peer, which);
  await mine.getByTestId("composer-identity-share").click();
  await expect(mine.getByTestId("composer-identity-done")).toBeVisible();
  await expect(mine.getByTestId("chat-identities-picker")).toHaveAttribute("data-side", "cards");
  if (verified) await expect(await myStatus(peer, which)).toHaveText("Shared · verified by your contact", { timeout: 60_000 });
