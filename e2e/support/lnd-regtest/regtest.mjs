#!/usr/bin/env node
// Drives the LND part of e2e/infra (Alice's and Bob's nodes on the regtest chain): worthless coins only, never a real node.
...
import { ensureMiner, mine, miner, retry, run as exec, wait } from "../../infra/chain.mjs";

export { mine };
export const LND_REGTEST = { alice: endpoints.lnd.alice, bob: endpoints.lnd.bob };
...
const service = (node) => { if (!(node in LND_REGTEST)) throw new Error(`No node ${node}`); return `lnd-${node}`; };
const run = (node, args) => exec(service(node), args);
export const lncli = (node, ...args) => JSON.parse(run(node, ["lncli", "--network=regtest", ...args]));
...
export async function ready() {
  ...
  if (!lncli("alice", "listchannels").channels.some((c) => c.remote_pubkey === bob && c.active)) {
    ...
    if (!lncli("alice", "listpeers").peers.some((p) => p.pub_key === bob)) lncli("alice", "connect", `${bob}@lnd-bob:9735`);
    const bothSynced = async () => { for (const node of ["alice", "bob"]) await until(`${node} synced`, () => lncli(node, "getinfo").synced_to_chain); };
    if (!lncli("alice", "pendingchannels").pending_open_channels.length) {
  ...
  for (const node of ["alice", "bob"]) {
    try { run(node, ["test", "-s", MACAROON]); } catch {
      run(node, ["sh", "-c", `lncli --network=regtest bakemacaroon ${SCOPE.join(" ")} > ${MACAROON}`]);
    }
  }
...
export function credentials(node) {
  return { url: LND_REGTEST[node], macaroon: run(node, ["cat", MACAROON]), cert: run(node, ["cat", "/root/.lnd/tls.cert"]) };
}
