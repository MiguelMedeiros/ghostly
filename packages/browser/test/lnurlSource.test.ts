 ...
 * amount chosen, checked against the mode, then quoted and paid like any pasted invoice.
 */
...
function service(fetch: typeof globalThis.fetch, fake = new FakeLightningProvider({ balance: 10_000 })) {
  ...
  return { fake, lightning: new LightningService(() => [cashuMint, descriptor], () => ({ platform: "web", cashu }), events, CASHU_MINT_SOURCE, { fetch }) };
}
...
describe("a Lightning address as a payment destination", () => {
  it("resolves to what a person sees, then fetches and checks the invoice for the amount chosen, and pays it through the source", async () => {
    ...
    const { lightning, fake } = service(fetch);
  ...
  it("refuses an invoice of a test network in Mainnet, and one the service made for another amount", async () => {
    const { fetch } = fakeFetch((sats) => testInvoice({ sats, descriptionHash: hash(), prefix: "lnbcrt" }));
    const { lightning } = service(fetch);
    ...
    const other = service(wrong.fetch).lightning;
    await other.start("mainnet");
