...
import type { LightningInvoice, LightningProvider } from "../../src/engine/paymentAdapters/providers/lightning";
import type { OnchainProvider } from "../../src/engine/paymentAdapters/providers/onchain";
import { NothingSpentError, type ProviderNetwork } from "../../src/engine/paymentAdapters/providers/types";
...
export function describeLightningProvider(name: string, make: () => Promise<LightningHarness>, { timeout = 10_000 } = {}) {
  describe(`${name}: the LightningProvider contract`, () => {
    ...
    it("refuses what it cannot pay with NothingSpentError, before anything leaves", async () => {
      ...
      await expect(provider.payInvoice(await refused(), 10)).rejects.toBeInstanceOf(NothingSpentError);
    });
  });
}
