...
/**
 * `bitcoin` (on-chain) and `fedimint` are never offered in the handshake: a full offer is already at the 16
 * capabilities apps before 0.5 accept. Both sides allow them only through the `paired-payments` list of the open
 * session.
 */
export type PaymentMethodName = "cashu" | "lightning" | "arkade" | "usdt" | "bark" | "bitcoin" | "fedimint";
...
export class PairedSession {
  ...
  peerAllowsPayment(method: PaymentMethodName): boolean {
    ...
    if (method === "arkade" || method === "usdt" || method === "bark" || method === "bitcoin" || method === "fedimint") return theirs.includes(capability);
