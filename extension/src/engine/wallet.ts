...
import type { MintView, StoredProof, StoredQuote, WalletView } from "../shared/types";
...
export class CashuWallet {
  ...
  async view(): Promise<WalletView> {
    ...
    return { mints, balance: total(mints.map((m) => ({ amount: m.balance }))) };
  }
   ...
   */
  async createToken(amount: number, preferred?: string[]): Promise<{ token: string; mint: string }> {
    assertAmount(amount);
  ...
  async receiveToken(token: string): Promise<{ amount: number; mint: string }> {
    let mint: string;
    try {
    ...
    return this.locked(mint, async () => {
      ...
      this.events.onChange();
      return { amount: total(proofs.map((p) => ({ amount: p.amount.toNumber() }))), mint };
    });
  }
  ...
  async payQuote(quoteId: string, mint: string): Promise<boolean> {
    return this.locked(mint, async () => {
      const wallet = await this.wallet(mint);
      ...
      try {
        ...
        return result.quote.state === "PAID";
      } catch (error) {
