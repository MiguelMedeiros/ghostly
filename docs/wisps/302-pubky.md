...
A grant session carries its own credential. A cookie session is the homeserver's HTTP-only cookie in the browser, a third-party cookie from Ghostly's page: Chromium keeps it, but WebKit drops it (Safari, and the WKWebView of the desktop app on macOS), so there the homeserver refuses the Ring session's write. The desktop app therefore sends a cookie session's requests to the homeserver (its `/session` and its `/storage/…` files, nothing else) through Rust, with a cookie jar for that one approval that never reaches the page and is dropped when the approval ends; the relay's requests and a grant session's stay the page's. In a browser that drops the cookie, Ghostly says so and points to Passport, rather than showing a bare 401.
...
4. The key's owner chose that host, so it is contacted only on port 443 (a record with any other port is refused), and only after its A and AAAA records, looked up through the person's DNS-over-HTTPS resolver, are all public addresses (the same check as a domain proof's `https` method). A proof never makes a contact's app knock on its own network.
5. `GET https://<host>/pub/ghostly.app/proofs/<folder>/<statement id>.txt` with header `pubky-host: <key>`, at most 1024 bytes. It must answer 200 with exactly the statement's text.

When step 4 or 5 fails because of what the contact's own network did (a host refused, not reached, or an error status), the contact's app keeps the detail and tells the person only that it could not check the proof: the host, address or status it saw never goes back in `idp-result`.
...
- **What contacts learn.** The Pubky key, which links every conversation it is shared in, and the proof key. Reading the proof, a contact's app contacts Pubky's Pkarr relays, its DNS-over-HTTPS resolver (which learns the homeserver's name) and the person's homeserver, which sees the contact's IP address.
...
- Pending: approval by the real Pubky Ring and the real passport.pubky.app, checked by hand. A Ring approval completes where the browser keeps the homeserver's cookie (Chromium: Chrome, Edge, the extension) and in the desktop app, which keeps it outside its WebView; not in Safari.
