...
| Implementation | Existing 1:1 calls and screen sharing in compatibility chats (Ghostly 0.4 contacts); calls in every chat being implemented |
...
Media transport is negotiated for its actual properties, not assumed to work because chat works over the adapter. Keep media and heavy signaling off the DHT after connection whenever possible. Bounded initial signaling may remain in Core.
