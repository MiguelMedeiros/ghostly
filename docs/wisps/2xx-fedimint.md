...
| Implementation | Experimental browser adapter on the Fedimint web SDK (`@fedimint/core` canary, pinned). Testnet only (New offers no Mainnet Fedimint yet); regtest evidence below. |
...
- The real SDK in Chromium (Playwright) against e2e/infra's federation (`fedimint/fedimintd:v0.12.1`, one guardian, v1 modules; `fedimint/gatewayd:v0.12.1` on LND 0.19.3; an LND peer with a 2,000,000-sat channel to the gateway's node) on Miguel's test server "one": two browser profiles joined federation `78b85e74c1ec9b21dd0f6d0efbbe2053a93efb8a662ea6e0af946852606a2e34` (set up from scratch by the seed in about 75 s; a first run on another federation gave the same figures) after its preview (name "Ghostly regtest", 1 guardian, consensus 2.1, regtest); the LND peer paid Alice's 20,000-sat gateway invoice (19,994 sats arrived); 1,000 sats of notes went to Bob's wallet page and were redeemed there, 500 were taken back; Alice's Send of 3,000 in the chat was reviewed as Fedimint ecash (her app asked Bob's), redeemed by Bob and settled his request; with both on the Fedimint Lightning source, Bob's 2,000-sat Lightning request was paid by Alice inside the federation; Alice paid a 3,000-sat invoice of the LND peer through the gateway (fee 13 sats, settled at the peer). Final balances: Alice 10,979, Bob 5,991 (6,000 received less the federation's redeem fees).
