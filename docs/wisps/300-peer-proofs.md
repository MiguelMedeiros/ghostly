...
**What this trades.** The external signer does not see which contact a proof is presented to: the person chooses per chat inside Ghostly, and a compromised Ghostly profile could present its proofs to anyone until they expire or are revoked. In exchange, publish-style (DNS, a Pubky homeserver file) and paste-back (SSH, PGP, Bitcoin) proofs are usable, and one OpenID login serves every contact.
...
**In the conversation (2026-09-26).** Each side's chat shows a share as an entry of its own timeline, made from the exchange's steps, with nothing new on the wire: the sharer's entry starts when it shares (checking; "waiting" while the contact is away) and settles on `idp-result`; the contact's starts on `idp-request` (the kind only) and settles on its own verification of `idp-present` (the identity, verified or not, with this side's reason). `idp-withdraw` (and a revocation found on re-check) adds a stop entry once; a share withdrawn while still queued never reached the contact and leaves none. One entry per share: a request repeated at a reconnect, or a second share of what the contact already verified, updates it in place; sharing again after a failure or a stop is a new entry. The entries are local state kept with the chat's identity ledger (at most 100, newest last, `timeline` in `IdentityLedger`, never sent). The challenge record keeps the requested proof id locally so the presented proof settles the right entry. They are not messages: never unread, never delivered. A share the contact made is the chat list's preview ("Shared an identity") and moves the chat up, until a newer message; a share of mine, or a stop, is neither.
