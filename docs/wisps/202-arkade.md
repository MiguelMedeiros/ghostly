...
This adapter implements Ark payments using `@arkade-os/sdk` 0.4.74. Ark is the payment method; Arkade is the selected provider implementation. [Bark](https://second.tech/docs/bark-sdk/index.md) remains an alternative provider candidate, not a separate currency or an automatically compatible adapter. Arkade was selected because its TypeScript wallet, transaction preparation, operator/indexer clients and browser repositories fit the existing browser engine. That choice does not establish parity across native, extension, mobile or hardware signers.
