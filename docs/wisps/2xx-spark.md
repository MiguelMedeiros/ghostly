...
The wallet is a BIP-39 phrase generated by Ghostly (or restored), sealed like the other wallets (AES-GCM, PBKDF2-SHA256 600,000 iterations) with a device key stored beside it. The Mainnet API key is sealed the same way. Each network has its own wallet (`sparkWallet-mode-<network>`, see [200](200-payments.md)), opened side by side. A wallet with money or payments is never replaced, and an empty one is archived (`sparkWallet-retired-*`), not deleted.
