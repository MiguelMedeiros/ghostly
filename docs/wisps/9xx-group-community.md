...
Beacon, lobbies and knocks are polled records, and on Pkarr relays every poll is a request against a budget: the web app allows itself 30 a minute per relay. Every read and write of these records is a **background** request: background requests may spend 20 of each relay's 30, counted on their own, and spend far less, because an admission's two links (the entry session, then the newcomer's edge) spend about nine of each relay's requests signaling, and a door must be able to let in a second person in the same minute. A lone door spends about 26 a minute on both relays together, 13 on each: the bell 15, the other knock records 2, its lobby 3, the beacon 6; with other hubs, four more reads of the beacon. An entry session closes without a last packet saying so, as soon as the joiner has closed its side. Before these rules, a hub's polling (knocks every 1.25 seconds, its lobby every three) took its whole budget once its app had been open a minute, and its side of an entry session waited 10 to 60 seconds for a request to get through. The same budget bounds how fast one door lets a crowd in through relays: about three people a minute at a few seconds each, the rest in turn.
