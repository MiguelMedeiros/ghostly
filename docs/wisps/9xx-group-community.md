...
3. **Concurrent commits are a race with a deterministic winner, not a fork.** The admin's changes are final: a branch that keeps one wins over a branch that drops it. Otherwise the longest branch wins; between branches of equal length, the one whose first commit has the lowest hash. The admin signing changes on two branches is still a fork and halts the group. To keep races rare, one hub at a time (the **door**: of the hubs that have been hubs for a minute, the one with the lowest key) answers knocks and commits leaves; the others step in, one at a time, only for someone still waiting.
...
Any member may commit, so two members can commit after the same commit before hearing of each other. Every member keeps the commits it has seen off its branch (up to 512) and follows the **best branch**. Two branches are compared on what each holds after their common parent:

1. **The admin's changes are final.** A branch that holds a `remove`, `role`, `rotate` or `link` commit beats one that holds none, however short it is and however far back they part. Otherwise any member could undo a removal, a hand-over, a rotation or a link turned off by signing, from a commit before it, more admissions than the admin made commits since (admissions of made-up keys cost nothing); the removed member would then be back in, and every later derived secret would come from one it holds.
2. Then the **longest**, however far back it parts from mine.
3. Then, between branches of the same length, the one whose first commit after their common parent has the **lowest hash**, if they part within the last 64 epochs.

Everyone who has seen the same commits follows the same branch. A commit that loses is not an error: a newcomer whose `add` lost is not a member and knocks again, which any member answers; a leave that lost is committed again by the next member that sees the leaver's request. Messages sent under a losing commit stay readable, because a message names its commit (below) and members keep the secrets of losing commits they could derive or were sent.
...
Two different commits signed by the **admin** after the same commit are equivocation: that is a fork, and the group halts on every member that sees both, as in the mesh. So is an admin change arriving on a branch that parts from mine before an admin change of mine: whoever was the admin where the two part signed the first admin change on each (the role only moves by an admin change), so they signed two histories. That covers an admin who handed the role on and then signs a change from before the hand-over. An honest admin never does it: it signs only on the branch it follows, and never leaves a branch holding its change for one that lacks it. A claim without a signed commit is answered with the commit.

Why not something cheaper against length:
...
An online member reads the beacon every ten seconds (never less often: hubs agree on who is at the door from the beacon, below; a hub alone in it, with nobody to agree with, reads it only when it republishes). With fewer than two fresh hubs, or all of them at capacity (48 members), and fewer than eight hubs, it becomes a hub after a random delay of up to three seconds (and a second reading), so that a crowd does not all step up; with no fresh hub at all it does so at once, since there is nobody to agree with and an extra hub steps down when idle. A newcomer first connects to the member who let it in, and considers being a hub only 30 seconds after its admission. Otherwise a member picks the least loaded hub and asks in its lobby, refreshing its entry every 20 seconds; a hub that has not opened its edge within 20 seconds is avoided for a while, and a member that no hub takes becomes one. A hub polls its lobby every six seconds for a minute after someone new asked there, every thirty otherwise, and opens an edge to every fresh key there that the chain has not taken out, up to its capacity (a member whose admission lost a race, or who is ahead of the hub's view of the roster, needs the edge to find out; the session hands nothing to a key that is not a member); hubs open edges to every other fresh hub in the beacon, and republish their entry every 30 seconds, or as soon as their load moves by eight. A hub with no members and nobody asking in its lobby for a minute steps down when two other fresh hubs remain and it has an edge up to one of them (a hub cut off from the others would only come back as one), closing any entry session it was running. A member whose admission lost a race stops being a hub at once and knocks again.
...
Beacon, lobbies and knocks are polled records, and on Pkarr relays every poll is a request against a budget: the web app allows itself 30 a minute per relay. Every read and write of these records is a **background** request: background requests may spend 20 of each relay's 30, counted on their own, and spend far less, because an admission's two links (the entry session, then the newcomer's edge) spend about ten of each relay's requests signaling, and a door must be able to let in a second person in the same minute. A lone door spends about 22 a minute on both relays together, 11 on each: the bell 15, the other knock records 1, its lobby 2, the beacon 6; with other hubs, four more reads of the beacon. An entry session closes without a last packet saying so, as soon as the joiner has closed its side. Before these rules, a hub's polling (knocks every 1.25 seconds, its lobby every three) took its whole budget once its app had been open a minute, and its side of an entry session waited 10 to 60 seconds for a request to get through. The same budget bounds how fast one door lets people in through relays: two in a minute at a few seconds each; a third in the same minute waits for the door's budget, and so does a crowd, in turn.
...
As in [the mesh profile](9xx-group-mesh.md#mentions): the same list of `{ "k", "o", "l" }` (member key, code point place, length with the `@`), the same rules for keeping an entry, at most 16. Here it travels **inside** the sealed payload, beside the text, so the author's signature covers it and a hub relays it without being able to change it:
...
The joiner does what it does in the mesh: a member key, knocks, an entry session toward the entry key. It draws its member key so that its end of the entry session (whose keys derive from both ends', as an edge's) is the higher one: only the lower key of a paired session offers, once it has seen the other side, so the member's side, which opens on seeing the knock and finds the joiner there, offers at once, two trips through Pkarr instead of three. Knocks go to four records (`<group id>.<n>` in place of the group id in the knock identity's derivation): the **bell**, `n` = 0, while it holds fewer than five fresh knocks of others, and otherwise the joiner's own record, `n` = 1 + (the first byte of its key modulo 3), so a crowd opening the link at once is not six at a time. The joiner refreshes its knock every ten seconds (every twenty after two minutes), where it put it, and keeps looking fast for the member's side of its entry session meanwhile. The door reads the bell every four seconds (every 2.5 for a minute after its app showed the link to be handed out; every six while an admission of its own is under way and for 30 seconds after) and one of the other records every minute; all of them at once when it becomes a hub, when its app comes back after a while, and when it becomes the door; and one of them with every read of the bell for two minutes after it found the bell full. The other hubs at the door read one record every fifteen seconds, in turn, for their turns below. Every member holds the entry key's seed (sealed in the welcome), so any member can answer, but never two at once: an entry session derives from the entry key and the joiner's key, the same for every hub, and two hubs answering one joiner collide on it and neither gets through. The hubs **at the door** are those that republished in the last 45 seconds and have been hubs for a minute (all of them, when none has), so that every hub, having read the beacon since, agrees on the set; the **door** is the lowest-keyed of them. Each attempt belongs to one hub, in turns of two minutes counted from when a hub first saw the knock with the current set of hubs at the door (when the set changes, say because the door's app closed, the count starts again and the new door answers at once, but only a joiner whose knock was refreshed in the last fifteen seconds, since the former door may still be letting it in; a joiner stops knocking as soon as it sees a member's side of its entry session): the door first, then the other hubs at the door, in the order they rank for this joiner (highest `SHA-256(knock key ‖ hub key)` first), round again. A hub opens a session only in the first 20 seconds of its turn, gives up on one nobody answered after 90 (a paired session over Pkarr can take the better part of a minute to come up), and after its first turn answers only a joiner still knocking. The member that lets someone in is a hub and its app is open: it takes the newcomer as one of its members at once and opens their edge, and the newcomer opens its side toward it, both looking for the other fast, with no lobby in between (the newcomer writes in the lobby only if that edge is not up in 20 seconds). An admission's entry session and this edge keep looking fast until they are up, for up to two minutes, so a short relay budget delays them by a poll rather than by the half minute of a background poll. Over the entry session: `group-invite`, `group-accept`, then the welcome, in pieces when the chain is long:
...
`group-mesh/1` groups, their links and their frames are unchanged. Community frames carry `v: 2`; an app from before metadata drops `group-meta` and relays none of it, and members that reach each other only through such a hub get the picture at their next sync with a newer member; a mesh session ignores them, and an app from before this profile never receives one, since it never announces 2. Community groups are not offered as contact invitations at all. An app of revision 0.2 drops a `group-msg` that opens to `x` or `p` (it expects `text`): it shows nothing and, as a hub, does not pass it on, so payments need the two members and the hubs between them on revision 0.3. Text is unaffected. Mentions (revision 0.5) ride inside the payload: an older app reads the text, ignores `m`, and relays the frame unchanged.

The final admin changes (revision 0.6) change only which branch a member follows, not a frame. An app of this revision that kept, under the old rule, a branch that dropped an admin change follows the admin's branch again when it starts, and its losing admissions knock again as after any race. An older app still follows the longer branch, so while a group mixes the two, a member crafting a longer branch can still move the older apps; they come back once updated. An older admin that followed a longer branch over its own change and then signs another change can halt the group on the newer apps (two histories); that needs the admin's change and a member's admission to race, and the admin to be on the older app.

## Bounds

256 members; 2048 commits; 8 hubs, 48 members per hub, one hub per member; 256 stored frames and 1 MiB (payment frames included); 96 epoch secrets; 512 commits off the main branch; ties between branches decided within 64 epochs; 16 KiB of text, mentions included (at most 16); 8 KiB of application frame or pair payload; 60 KiB per frame; four knock records and lobbies of six entries each. The member cap is what the load test below measured; see Conformance.
...
|---|---|---|
| Admission of everyone through the link, in waves of 16, the admin's app closed after the first | 96 s simulated | 742 s simulated (about 21 a minute; 452 to 1248 s over four runs); 346 s with the knock bell and pacing of revision 0.4 |
...
This measures the protocol, the topology rules and the cryptography at the cap. It does not measure what only real networks show: WebRTC between hundreds of browsers (the e2e has six), a browser holding the fifty-odd peer connections of a busy hub, public Pkarr relays' rate limits (the 256-member run made 18 035 Pkarr reads and writes in total, 11 050 with the pacing of revision 0.4), or an entry session over the public relays, which on the loaded test machine took from 5 to 80 seconds to come up. The cap is therefore a measured bound for the protocol and a stated one for the network.

### Joining, timed

[`communityJoinTiming.test.ts`](../../packages/browser/test/communityJoinTiming.test.ts) times a join on the headless engines with the network costing what relays cost (each app's budget, a link up only after both sides polled each other at the link's pace); [`group-join-timing.spec.ts`](../../e2e/web/group-join-timing.spec.ts) in real browsers (`E2E_JOIN_WARM_MS` is how long the member's app was open). From opening the link, median / 90th percentile, on 2026-09-24:

| Real browsers (`group-join-timing.spec.ts`), median (range) | Runs before / after | Knock seen | In the group | A member reached |
|---|---|---|---|---|
| Member's app open 6 s, test relay | 8 / 8 | 3.5 s (1.1-7.8) → 1.9 s (0.2-3.4) | 9.7 s (3.8-11.7) → 6.3 s (4.0-10.8) | 16.6 s (10.2-29.4) → 11.4 s (8.7-19.1) |
| Member's app open 2.5 min, test relay | 6 / 6 | 6.4 s (2.4-7.0) → 1.9 s (0.6-3.3) | 21.9 s (15.0-28.4) → 6.7 s (4.8-9.5) | 62.1 s (26.3-75.2) → 11.7 s (9.7-14.7) |
| Member's app open 2.5 min, public relays | 4 / 5 | 15.0 s (6.7-21.9) → 3.6 s (2.1-4.2) | 22.0 s (17.3-31.6) → 12.0 s (9.4-15.5) | 50.9 s (34.8-62.2) → 22.4 s (18.9-26.9) |

| Headless, relay costs (`communityJoinTiming.test.ts`), p50 / p90 of 30 runs | In the group | Edge to a hub up |
|---|---|---|
| One member's app open a while | 4.5 / 6.5 s (before: not in within 300 s) | 9 / 11 s |
| The only member's app opens as the link is opened | 4.5 / 4.5 s | 9 / 9 s |
| A second joiner right after the first | 6.5 / 6.5 s | 11 / 11 s |
| Four members online | 4.5 / 6.5 s | 9 / 11 s |
