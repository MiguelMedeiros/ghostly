...
A message may name members: "@Bob, are you in?" notifies Bob. Names can collide and change, so a mention is bound to the **member key**, not the name. The text keeps a readable `@Name` at each place, so an app without mentions shows the message as it was typed; a list says which member each place is:
...
```
{ "t": "group-msg", …, "sig", "m"?: { "n": <nonce>, "c": <box> } }
```

It is sealed with XChaCha20-Poly1305 under the same epoch message key, with a nonce of its own and the JSON of `["ghostly-group/1 mentions", g, e, s, n, ts]` as associated data, so it opens only on its own message. It is outside the signature, which stays exactly the one older apps check. In this profile only the author's edge ever carries its frame (a receiver takes a frame only from the edge of its sender), so the edge vouches for the box, and a member who could tamper with it could only have written it. A box that does not open, or opens to something else, is no mentions; the message is taken all the same. The author keeps the box with the frame, so catch-up carries it.

**Everyone.** `*` names every member, and is kept only when the sender was the admin of the message's epoch; the composer offers `@everyone` to the admin only. A member's `*` is dropped by the sender's own app and by every receiver.

**Notifications.** A message that names the reader (or everyone) is a mention. It notifies as any message does. While the group is muted it still does, if the group's "Still notify me when I'm mentioned" is on, as it is unless turned off. That choice, like the mute, is the device's and is never sent. An unread mention puts an `@` beside the group in the chat list.
...
A group has **metadata** besides its roster: today, a picture. It is not in the membership chain: a new commit kind or field would make every older app refuse the chain. It travels in a frame of its own, which older apps drop, and it is **the admin's word**: only the admin sets or removes it, and every member keeps it and hands it on.

```
{ "t": "group-meta", "g", "e": <epoch of the commit signed under>, "h": <its hash>, "r": <revision>, "by": <admin key>, "ts",
  "d": <SHA-256 of the body>, "sig", "k": <epoch whose message key seals the body>, "nn": <nonce>, "c": <box> }
```

**The statement.** The admin signs `["ghostly-group meta", g, e, h, r, by, ts, d]` with its member key. `(e, h)` is a commit of the chain the signer is the admin after; `r` counts up from 1. `d` is the SHA-256 (hex) of the **body**, the metadata as JSON: `{ "pic": <data URL> }`. The body is the whole metadata: a field left out is unset, so `{}` removes the picture. Unknown fields are ignored.
...
1. It comes from a member's edge.
2. It is **newer** than the one held: signed under a later commit, or a higher revision under the same one.
3. Its commit `h` is on the member's chain at index `e`.
4. Its signer was the admin after that commit **and is the admin now**.
5. The signature holds.
6. It opens under epoch `k` and hashes to `d`.
7. Its body is one Ghostly shows.

A statement naming a commit or epoch the member does not have yet waits (one at a time), while the member asks the sender to catch it up. A statement already taken is kept when the admin changes. Condition 4 means a former admin cannot overrule the new one by signing under a commit from before its demotion. Because of it, **a new admin signs the current metadata again**, under its own commit and with the next revision, as soon as it applies the commit that made it admin, so members who join afterwards (who accept only the current admin's statement) still get it.

**Delivery.** The admin sends the frame to every member when it sets or removes the picture. Sync frames carry `"mt": "<e>.<r>"` (`""` for none): the statement the sender holds. The side holding a newer one answers with it, sealed under its current epoch, after the commits and secrets the other lacks. That is how a member who was away, and a member admitted later, by a contact or by the link, gets the picture without the admin. The picture is shown after admission: a link carries nothing that would let a joiner fetch it before. An app from before metadata sends no `mt` and is sent nothing; it keeps showing the group glyph.

Each change leaves an event line in the group's history ("Alice changed the group's picture"). System notifications stay as they are, with no group name or picture: they say only that something arrived.

## Compatibility

Groups are announced after the paired handshake, on the open session, as `{ "t": "paired-groups", "v": [1] }`, the way `paired-payments` is, so a full handshake offer stays within the sixteen capabilities older apps accept. An app without groups drops the announcement and every `group-*` frame (they carry no `id`), keeps chatting 1:1, and is shown as needing an update to be invited. No 1:1 behavior changes. Payments on edges came after groups (revision 0.3): an app from before offers none on its edges, drops `group-pay`, and simply sees no payments; nothing else changes for it. Metadata came in revision 0.4: an app from before drops `group-meta`, leaves `mt` out of its syncs and is sent none, and its chain rules are unchanged. Mentions came in revision 0.5: an app from before ignores `m` (it checks the fields it knows, and the signature covers only those), shows the `@Name` as text and notifies as for any message.

## Bounds

Eight members; 16 mentions per message, each at most 65 code points; one metadata statement per group, its picture at most 40,000 characters; 32 payment notes said again per edge; 1024 commits; 16 KiB of text; 32 own messages and 128 KiB kept for catch-up; 64 waiting frames and 1 MiB; 16 epoch secrets; a replay window of 256 sequence numbers per sender and epoch; 16 commits buffered ahead of the chain; 24 commits per welcome piece; 60 KiB per frame. These are the first profile's numbers, chosen to be small, not measured product limits.
