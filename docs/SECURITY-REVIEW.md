...
One gap worth naming: the push of this branch was answered with "GitHub found 1 vulnerability on the default branch (1 moderate)", and the routine cannot see what it is. The token it runs with has no `security_events` scope, so `dependabot/alerts` answers 403, and the global advisory endpoints are not reachable from here either. Nothing in the three lock files accounts for it — both npm trees are clean and the Rust lock has no vulnerability, only the eight accepted warnings — and the allowlist already carries a GHSA id (`GHSA-wrw7-89jp-8q8g`, the `glib` unsoundness), which is the form Dependabot alerts take, so that entry or the `lru` one is the likely match. Both are accepted and upstream-blocked under S19. Confirming it rather than inferring it needs either a token with that scope or someone reading the alert; until then the routine's dependency picture is one source short, and this note is where that is recorded.
