...
> **Decided (2026-09-25), being implemented.** One chat and one invite: every chat runs this DHT delivery from its first contact, in parallel with a stream attempt, falls back to it whenever no stream connects, and upgrades to a stream by itself. `pair1/` and `pair2d/` give way to one bech32m `ghostly1…` invite (shared as `https://ghostly.tools/#ghostly1…`), and DHT only becomes a per-chat choice instead of an invite type. See [WISP 400](wisps/400-chat.md), [WISP 403](wisps/403-dht-text.md) and [WISP 801](wisps/801-invitation-profiles.md). This document still describes the code as it is.
