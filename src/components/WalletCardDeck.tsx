 ...
 * checks (the ways of paying a chat accepts) `checked` says whether it is on: in its colours, or grey. `corner` takes
 * the chip's place (that deck's switch). A test card says Testnet; `tagAll`: a real one says Mainnet too, where both
 * kinds share a deck (the chat's).
 */
export function WalletCardFace({card,after,checked,tagAll,corner}:{card:WalletCard<string>;after?:boolean;checked?:boolean;tagAll?:boolean;corner?:ReactNode}) {
 return <span className="wallet-deck-face" data-deck="face" data-after={after||undefined} data-checked={checked}>
  ...
  {corner??<span className="wallet-deck-card-chip" aria-hidden="true"/>}
...
export interface CardDeckProps<Id extends string=WalletRail> {
 ...
 /** A card's accessible name, when its face does not say what choosing it does (deck/Deck.tsx). */
 cardLabel?:(card:WalletCard<Id>)=>string;
 /** What a card shows in its chip's place (the Accept deck's switch). */
 corner?:(card:WalletCard<Id>)=>ReactNode;
 label:string;
 testId:(id:Id)=>string;
...
export function CardDeck<Id extends string=WalletRail>({onSelect,onChoose,testId,tagAll,corner,...props}:CardDeckProps<Id>) {
 return <Deck<WalletCard<Id>> {...props} className="wallet-deck" panel={WALLET_PANEL}
  ...
  face={(card,{after,checked})=><WalletCardFace card={card} after={after} checked={checked} tagAll={tagAll} corner={corner?.(card)}/>} mark={card=><WalletMark rail={railOf(card)}/>} tone={card=>`wallet-card-${railOf(card)}`}/>;
}
