...
export function PaymentReview({review:initial,wallet,onClose}:{review:Review;wallet:WalletPlatform;onClose:()=>void}) {
 ...
 const approve=()=>void run(()=>wallet.approvePayment(review.id)).then(()=>setConfirming(false));
 ...
 return <section aria-label="Payment review" className="rounded-xl border border-border p-3 space-y-3" data-testid="payment-review" data-network={network}>
  ...
  {review.state==='pending'&&confirming&&<div className="rounded-lg border border-danger/50 p-2 space-y-2" data-testid="review-mainnet-confirm" role="group" aria-label="Confirm real money">
   <p className="text-xs text-text-primary m-0"><strong>{MONEY_LABEL.mainnet}.</strong> This sends {shown} {unit} that cannot be taken back once it settles. Nothing has gone out yet.</p>
   <div className="flex gap-2 flex-wrap"><button className={`${button} !bg-accent !text-on-accent`} data-testid="review-confirm-send" disabled={busy} onClick={approve}>Send real money</button><button className={button} disabled={busy} onClick={()=>setConfirming(false)}>Back</button></div>
  </div>}
