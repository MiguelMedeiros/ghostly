...
export function UsdtWalletPanel({wallet,state}:{wallet:WalletPlatform;state:WalletState}) {
 ...
 return <div className="space-y-6" data-testid="usdt-wallet" aria-label="USDT wallet">
  ...
  :!ready?<Section title="Unlock"><Row label="This wallet was created with a password"><input aria-label="USDT wallet password" type="password" autoComplete="current-password" className={input} value={password} onChange={e=>setPassword(e.target.value)}/><Button variant="primary" disabled={busy} onClick={()=>void run(async()=>{await wallet.usdtUnlock(password);setPassword('');})}>Unlock USDT</Button></Row></Section>
  :<div className="space-y-4">
   <div>
    <p className="text-text-primary" data-testid="usdt-balance"><span className="text-4xl font-semibold tabular-nums">{formatPaymentAmount(usdt.balance,usdt.decimals)}</span> <span className="text-text-muted text-sm">{label}</span></p>
   ...
   {action==='send'&&<div className="bg-surface rounded-xl p-4 space-y-3 animate-fade-in">
    ...
    <label className="flex items-center justify-between gap-3 text-sm text-text-secondary">Network fee limit<span className="flex items-center gap-2"><input aria-label="Maximum gas in ETH" inputMode="decimal" className={`${input} w-28 text-right`} value={gas} onChange={e=>setGas(e.target.value.replace(/[^0-9.]/g,''))}/>ETH</span></label>
  ...
  {(ready||stuck)&&<Section title="Settings">
   ...
   {pending&&<Block>
    ...
    <div className="flex gap-2"><Button variant="primary" disabled={busy||!token} onClick={()=>void run(async()=>{await use(pending,{provider,token});setPending(null);})}>Switch network</Button><Button onClick={()=>setPending(null)}>Cancel</Button></div>
   </Block>}
   {ready&&<><Row label="RPC provider" hint={`${usdt.provider} · sees your public address`}/>
   <Row label="Token" hint={`${usdt.token} · ${usdt.decimals} decimals${usdt.chainId===1?' · Tether can freeze USDT':''}`}/>
