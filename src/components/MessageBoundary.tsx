...
export function Unshowable({ text, fromMe }: { text: string; fromMe: boolean }) {
  const { t } = useI18n();
  return (
    <div data-testid="message-unshowable" data-message-row className={`flex ${fromMe ? "justify-end" : "justify-start"} mb-3.5 px-[63px] max-md:px-2.5`}>
      <div className={`max-w-[85%] rounded-[7.5px] px-[9px] pt-[6px] pb-[8px] text-text-primary text-[14.2px] leading-[19px] ${fromMe ? "bg-sent-bg" : "bg-received-bg"}`}>
        ...
        {text && <p dir="auto" className="mt-1 whitespace-pre-wrap wrap-break-word">{text}</p>}
      </div>
    </div>
  );
}
 ...
 */
export class MessageBoundary extends Component<{ text: string; fromMe: boolean; children: ReactNode }, { failed: boolean }> {
  state = { failed: false };
