...
export function DeleteProfileDialog({ entry, onClose }: { entry: ProfileEntry; onClose: () => void }) {
  ...
  const button = "px-4 py-2 rounded-lg text-sm cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-fade-in" {...backdrop}>
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="delete-profile-title" data-testid="delete-profile" className="focus:outline-none w-full max-w-sm bg-panel-header border border-border rounded-2xl shadow-2xl p-5 space-y-4">
        <div className="space-y-1">
          <h2 id="delete-profile-title" className="text-base font-semibold text-text-primary">Delete “{entry.name}”?</h2>
        ...
        {!backingUp ? (
          <button type="button" onClick={() => setBackingUp(true)} className="text-sm text-accent hover:underline cursor-pointer">{saved ? "Backup downloaded ✓" : "Download a backup first"}</button>
        ) : (
