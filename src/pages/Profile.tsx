...
function LinkRow({ label, value, onClick }: { label: string; value?: ReactNode; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-surface-alt transition-colors cursor-pointer first:rounded-t-xl last:rounded-b-xl">
      <span className="flex-1 text-sm text-text-primary">{label}</span>
      {value !== undefined && <span className="text-sm text-text-muted tabular-nums">{value}</span>}
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" className="text-text-muted" aria-hidden="true"><path d="m9 6 6 6-6 6" /></svg>
    </button>
  );
}
...
export function Profile() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-chat-bg overflow-hidden min-h-0" data-testid="profile-page">
      <header className="h-14 header-safe shrink-0 bg-panel-header flex items-center px-4 border-b border-border">
        ...
        <h1 className="text-lg font-medium text-text-primary">{t("settings.profile")}</h1>
      </header>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="flex items-center gap-4">
            ...
            <input data-testid="profile-name" aria-label="Profile name" value={name} maxLength={32} onChange={(e) => setName(e.target.value)} onBlur={saveName} onKeyDown={(e) => e.key === "Enter" && (e.currentTarget as HTMLInputElement).blur()}
              className="min-w-0 flex-1 bg-transparent text-xl font-semibold text-text-primary rounded-lg px-2 -mx-2 py-1 border border-transparent hover:border-border focus:border-accent focus:outline-none" />
            {myAvatar && <button type="button" data-testid="profile-avatar-remove" onClick={() => void setMyAvatar(null)} className="text-xs text-text-muted hover:text-danger cursor-pointer shrink-0">Remove picture</button>}
          </div>
          {error && <Notice tone="error">{error}</Notice>}

          <Section title="Look">
            <Row label="Color">
              <div className="flex gap-2" role="radiogroup" aria-label="Profile color">
                {COLOR_THEME_OPTIONS.map((option) => (
                  <button key={option.value} type="button" role="radio" aria-checked={settings.colorTheme === option.value} aria-label={t(option.labelKey as Parameters<typeof t>[0])} title={option.description} data-testid={`profile-theme-${option.value}`}
                    ...
                    className={`w-7 h-7 rounded-full border-2 transition-transform cursor-pointer ${settings.colorTheme === option.value ? "border-text-primary scale-110" : "border-transparent hover:scale-105"}`}
            ...
            <Row label="Name in chats">
              <input data-testid="account-nickname" aria-label={t("settings.nicknamePlaceholder")} className={`${input} w-44 max-sm:w-full`} value={settings.defaultNickname} maxLength={20} placeholder="Anonymous" onChange={(e) => updateDefaultNickname(e.target.value)} />
              <button type="button" onClick={randomizeNickname} title={t("settings.randomizeName")} aria-label={t("settings.randomizeName")} className="p-2 rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-alt cursor-pointer">↻</button>
            </Row>
          </Section>
          ...
          <Section title="Profiles" testId="profile-list">
            {all.map((entry) => (
              <div key={entry.id || "default"} className="flex items-center gap-3 px-4 py-2.5" data-testid="profile-row">
                ...
                {entry.id === current.id ? <span className="text-xs text-accent">In use</span> : <>
                  <Button data-testid="profile-switch" disabled={!canSwitch} onClick={() => attempt(() => switchProfile(entry.id))}>Switch</Button>
                  {entry.id && (
                    <button type="button" data-testid="profile-delete" aria-label={`Delete ${entry.name}`} title="Delete" onClick={() => setDeleting(entry)} className="p-2 rounded-lg text-text-muted hover:text-danger hover:bg-surface-alt cursor-pointer">
            ...
            {!canSwitch ? <Block><Notice>One profile only in this client, for now.</Notice></Block> : creating ? (
              <Block>
                <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); attempt(() => { const entry = createProfile(newName); switchProfile(entry.id); }); }}>
              ...
              <button data-testid="profile-new" onClick={() => setCreating(true)} className="w-full px-4 py-3 text-left text-sm text-text-secondary hover:text-accent hover:bg-surface-alt transition-colors cursor-pointer rounded-b-xl">+ New profile</button>
            )}
          </Section>
        </div>
      </div>
