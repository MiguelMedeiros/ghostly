...
export function Services() {
  ...
  return (
    <Page title="Services" testId="my-services" trailing={
    ...
    <div data-testid="service-item">
      <Row leading={<span className={`grid place-items-center w-10 h-10 rounded-xl ${shared ? "bg-accent/15 text-accent" : "bg-surface-alt text-text-muted"}`}>{GLOBE}</span>}
        ...
        hint={<span className={`flex flex-wrap items-center gap-x-1.5 mt-0.5 ${shared ? "text-accent" : "text-text-muted"}`}>
          ...
          {service.requests > 0 && <span className="text-text-muted">· {service.requests} requests</span>}
        </span>}>
          ...
          <button type="button" aria-label={`Remove ${service.name}`} title="Remove" onClick={() => void platform.removeService(service.id)} className="grid place-items-center w-10 h-10 rounded-lg text-text-muted hover:text-danger hover:bg-surface-alt transition-colors cursor-pointer">
