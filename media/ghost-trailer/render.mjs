...
//   node media/ghost-trailer/render.mjs [--format 16x9|9x16|1x1] [--fps 60] [--workers 4]      → out/<format>/picture.mp4
//   node media/ghost-trailer/render.mjs --stills 0,3.3,13.4 [--format …]                        → out/<format>/stills/*.jpg
//   node media/ghost-trailer/render.mjs --sheet [--format …]                                    → out/<format>/sheet.jpg (24 stills across the film)
...
const { duration: DURATION } = JSON.parse(readFileSync(join(here, "out", "timeline.json"), "utf8"));
...
const server = await createServer({ configFile: join(here, "vite.config.ts"), server: { port: Number(process.env.FILM_PORT ?? 5393), hmr: false, watch: null } });
await server.listen();
...
async function open() {
  ...
  // The picture says where its sounds are: mix.mjs places them from this.
  writeFileSync(join(out, "sounds.json"), JSON.stringify(await page.evaluate(() => window.sounds), null, 1));
  return page;
}
...
try {
  if (process.argv.includes("--stills") || process.argv.includes("--sheet")) {
    ...
    const times = sheet ? Array.from({ length: 24 }, (_, i) => Number(((i + 0.5) * DURATION / 24).toFixed(3))) : arg("stills").split(",").map(Number);
