 ...
 */
export function WalletCardFace({card,after,checked}:{card:WalletCard;after?:boolean;checked?:boolean}) {
 ...
 return <span className="wallet-deck-face" data-deck="face" data-after={after||undefined} data-checked={checked}>
  <span className="wallet-deck-card-glyph" aria-hidden="true"><WalletMark rail={card.id}/>{check}</span>
  ...
  <span className="wallet-deck-card-glyph-end" aria-hidden="true"><WalletMark rail={card.id}/>{check}</span>
...
export interface CardDeckProps {
 ...
 checked?:(card:WalletCard)=>boolean;
 label:string;
...
export function CardDeck({onSelect,onChoose,testId,...props}:CardDeckProps) {
 return <Deck<WalletCard> {...props} className="wallet-deck" panel={WALLET_PANEL}
  ...
  face={(card,{after,checked})=><WalletCardFace card={card} after={after} checked={checked}/>} mark={card=><WalletMark rail={card.id}/>} tone={card=>`wallet-card-${card.id}`}/>;
}
