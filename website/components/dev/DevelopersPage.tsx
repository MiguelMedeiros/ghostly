  ...
  return (
    <Shell locale={locale}>
      <section className="devhero">
        <div className="wrap devhero-grid">
          <div className="devhero-copy">
            <span className="eyebrow">{t.hero.eyebrow}</span>
            <h1 className="h-display devhero-title">
              {t.hero.title1} <span className="accent">{t.hero.title2}</span>
            </h1>
            <p className="lead">{t.hero.lead}</p>
            <div className="devhero-actions">
      ...
      <section className="section section--tight" id="vocabulary">
        <div className="wrap">
          ...
          <div className="words">
            {t.words.items.map((w) => (
              <Reveal as="article" key={w.id} className="card word">
                <span className="word-icon">
                  <Icon name={WORD_ICONS[w.id]} />
                </span>
                <h3>{w.term}</h3>
                <p className="word-says">{w.says}</p>
                <p className="muted">{w.body}</p>
                <p className="word-example">
                  {"level" in w && w.level ? <LevelBadge level={w.level} locale={locale} small /> : <span className="mono">{w.example}</span>}
                </p>
              </Reveal>
            ))}
          </div>
      ...
      <section className="section section--tight compose-section" id="compose">
        <div className="wrap">
          <Reveal className="section-head">
      ...
      <section className="section section--tight" id="negotiate">
        <div className="wrap">
          <Reveal className="section-head">
      ...
      <section className="section section--tight" id="path">
        <div className="wrap">
          <Reveal className="section-head">
          ...
          <ol className="path">
            {t.path.steps.map((s, i) => (
              <Reveal as="article" key={s.title} className="path-step">
                <span className="path-num mono">{String(i + 1).padStart(2, "0")}</span>
                <div className="path-body">
                  <h3>{s.title}</h3>
                  <p className="muted">{s.body}</p>
                  {"code" in s && s.code && (
                    <figure className="path-code">
                      <pre>
      ...
      <section className="section section--tight" id="availability">
        <div className="wrap">
          <Reveal className="section-head">
